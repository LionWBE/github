//version 0.10 date 01/11/2022 это некорректная версия на стеке - вылетает
#include "lib_Tags.h"
...
void MyClass_MQTT_Tag::setup(MyClass_Config *my_config){
  ...
  col_tag = 0;
  cur_tagLink = 0;
  MaxCountTag = 200;  // править для увеличения - уменьшения количество доступных тегов (сделано статикой, чтобы не тормозить с обработкой динамических массивов или стека)
  MaxSizeStack = 200;
  TagNames = new char*[MaxCountTag];
  TagValsOld = new char*[MaxCountTag];
  TagUseTimeStamps = new bool[MaxCountTag];

  settings->status.MQTT_Tag.version_lib = "0.10";
