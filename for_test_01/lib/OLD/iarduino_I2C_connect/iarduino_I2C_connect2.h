...
class iarduino_I2C_connect{
	public:								
		...
		TwoWire *line;
		uint8_t	readByte		(uint8_t, uint8_t);				//	чтение одного байта данных из устройства с указанием: (адрес устройства, адрес регистра)
		uint8_t	writeByte		(uint8_t, uint8_t, uint8_t);	//	запись одного байта данных в  устройство с указанием: (адрес устройства, адрес регистра, байт для записи)
	...
	private:							
		/**	внутренние функции **/			
		void	I2C_func_BEGIN	(uint8_t*, uint8_t);			//  внутренняя функция
		void	I2C_func_MASK	(bool*,    uint8_t);			//  внутренняя функция
};
