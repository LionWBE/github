#include "iarduino_I2C_connect2.h"
#include "Wire.h"
...
void		I2C_func_ON_DATA(int i){																//	количество принятых байт по шине I2C
				...
				I2C2VC.I2C_index_REG=Global_line->read();													//	первый байт данных, полученный от мастера, является номером элемента массива (типа адресом регистра)
				for(int j=1; j<i; j++){																//	если получено более 1 байта данных, значит производится запись в массив (регистры)
					...
					if(k){                                                                          //  если маскировочный массив I2C2VC.I2C_array_MASK разрешает запись в очередную ячейку массива I2C2VC.I2C_array_REG
						I2C2VC.I2C_array_REG[I2C2VC.I2C_index_REG]=Global_line->read();                     //  записываем данные полученный байт данных в ячейку массива I2C2VC.I2C_array_REG
...
void		I2C_func_REQUEST(){																		//	без параметра
				if(I2C2VC.I2C_index_REG>=I2C2VC.I2C_length_REG){I2C2VC.I2C_index_REG=0;}			//	корректируем значение индекса активного элемента массива I2C2VC.I2C_array_REG, если он вышел за его пределы
				Global_line->write(I2C2VC.I2C_array_REG[I2C2VC.I2C_index_REG]);								//	читаем значение активного элемента массива I2C2VC.I2C_array_REG (элемент на который указывает индекс I2C2VC.I2C_index_REG)
				I2C2VC.I2C_index_REG++;																//	увеличиваем значение индекса активного элемента массива I2C2VC.I2C_array_REG
}
...
void		iarduino_I2C_connect::I2C_func_BEGIN(uint8_t *i, uint8_t j){							//	указатель на массив, длина массива
				...
				Global_line = line;
				line->onReceive(I2C_func_ON_DATA);													//	назначаем функцию для обработки принятых по шине I2C данных
				line->onRequest(I2C_func_REQUEST);													//	назначаем функцию для обработки запрошенных по шине I2C данных
...
uint8_t		iarduino_I2C_connect::readByte(uint8_t i, uint8_t j){									//	адрес устройства, адрес регистра
				...
				line->beginTransmission(i);															//	инициируем передачу данных по шине I2C к устройству с адресом i (При этом сама передача не начинается)
				line->write(j);																		//	указываем, что требуется передать байт j - адрес регистра из которого требуется прочитать данные
				line->endTransmission(true);															//	выполняем инициированную передачу данных (бит RW=0 => запись)
				if(line->requestFrom(i,uint8_t(1))){													//	запрашиваем 1 байт данных от устройства с адресом i, функция Wire.requestFrom() возвращает количество принятых байтов
					while(line->available()){k=line->read();}											//	читаем очередной принятый байт, если таковой имеется в буфере для чтения
...
uint8_t		iarduino_I2C_connect::writeByte(uint8_t i, uint8_t j, uint8_t k){						//	адрес устройства, адрес регистра, байт для записи
				line->beginTransmission(i);															//	инициируем передачу данных по шине I2C к устройству с адресом i (При этом сама передача не начинается)
				line->write(j);																		//	указываем, что требуется передать байт j - адрес регистра из которого требуется прочитать данные
				line->write(k);																		//	указываем, что требуется передать байт k - данные для записи в регистр j устройства i
				return line->endTransmission();														//	выполняем инициированную передачу данных (бит RW=0 => запись)
}
