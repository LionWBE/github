...
public:
    ...
    void setting(uint8_t in_DS_PIN, uint8_t in_DS_ADDR, uint8_t in_DS_AM = 1, bool in_DS_PGM = 0){
        ...
        state = new bool[DS_AM];
        _buf = new int16_t[DS_AM];
        _addr = DS_ADDR;
    ...
    bool readTemp(uint8_t idx = 0) {
        state[idx] = 1;                             // 1us
        if (!oneWire_reset(DS_PIN)) return 0;       // датчик оффлайн   1260us (тут выставлена жестко задержка в 1260us в функции)
        addressRoutine(idx);                   		// Процедура адресации 4800us
        oneWire_write(0xBE, DS_PIN);                // Запросить температуру  537us
        uint8_t crc = 0;                            // обнуляем crc   0us
        int16_t temp;                               // переменная для расчёта температуры
        uint16_t sum = 0;                           // контрольная сумма
        for (uint8_t i = 0; i < 9; i++) {           // Считать RAM  5300us
...
private:
    ...
    void  
    addressRoutine(uint8_t idx) {              // Процедура адресации
